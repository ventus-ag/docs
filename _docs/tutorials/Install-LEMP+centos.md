...
2) On the following page fill next information:
 - Instance Name (eg. "Test_LEMP" or "Test-Centos-LEMP)
...
1) Again, we can use yum to acquire and install our software. This time, we’ll also install some other “helper” packages that will assist us in getting our components to communicate with each other:
```
...
```
3) By default, our MariaDB is not secure by default, we need to remove the anonymous user and test database and configure other settings by running the command below:
...
```
4) Your MariaDB server is now secure. Type the command below to enable it during boot:
